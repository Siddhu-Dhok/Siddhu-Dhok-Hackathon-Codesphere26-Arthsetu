...
function utcMonthKey(tsMs: number): string {
  const d = new Date(tsMs);
  const y = d.getUTCFullYear();
  const m = String(d.getUTCMonth() + 1).padStart(2, "0");
  return `${y}-${m}`;
...
function utcQuarterKey(tsMs: number): string {
  const d = new Date(tsMs);
  const y = d.getUTCFullYear();
...
function getLimitFromUserDoc(
  ...
  if (key === FIELD_DAILY_LIMIT) return asNumberOrZero(userSnap.get(FIELD_EXPENSE_LIMIT_DAILY));
  if (key === FIELD_WEEKLY_LIMIT) return asNumberOrZero(userSnap.get(FIELD_EXPENSE_LIMIT_WEEKLY));
  if (key === FIELD_MONTHLY_LIMIT) return asNumberOrZero(userSnap.get(FIELD_EXPENSE_LIMIT_MONTHLY));
  if (key === FIELD_QUARTERLY_LIMIT) return asNumberOrZero(userSnap.get(FIELD_EXPENSE_LIMIT_QUARTERLY));
  return 0;
}

function getCategoryLimitsMap(userSnap: admin.firestore.DocumentSnapshot): Record<string, number> {
...
function limitForCategory(limits: Record<string, number>, categoryName: string): number {
  const trimmed = categoryName.trim();
  if (trimmed.length > 0 && limits[trimmed] > 0) return limits[trimmed];
...
function stableNotificationId(base: number, salt: string): number {
  let h = 0;
...
function maskToken(token: unknown): string {
  if (typeof token !== "string" || token.length === 0) return String(token);
...
export const onTransactionCreatedAggregateAndNotify = functions
  ...
  .onCreate(async (snap, context) => {
    ...
    const totalsResult = await db.runTransaction(async (tx) => {
      ...
      const prevQuarterTotal =
        prevQuarterKey === quarterKey ? asNumberOrZero(totals[FIELD_QUARTERLY_TOTAL]) : 0;
      ...
      const categoryMonthlyAll =
        (totals[FIELD_CATEGORY_MONTHLY_TOTALS] as Record<string, Record<string, number>>) || {};
    ...
    type PeriodKind = "daily" | "weekly" | "monthly" | "quarterly";
    const periodAlerts: Array<{ period: PeriodKind; limit: number; total: number; nid: number }> = [];
    ...
    const periodWarnings80: Array<{ period: PeriodKind; limit: number; total: number; nid: number }> =
      [];
    ...
    push80IfCrossed(
      "daily",
      dailyLimit,
      totalsResult.daily_total,
      prevDailyTotal,
      `80-daily-${uid}-${totalsResult.dayKey}`
    );
    push80IfCrossed(
      "weekly",
      weeklyLimit,
      totalsResult.weekly_total,
      prevWeeklyTotal,
      `80-weekly-${uid}-${totalsResult.weekKey}`
    );
    push80IfCrossed(
      "monthly",
      monthlyLimit,
      totalsResult.monthly_total,
      prevMonthlyTotal,
      `80-monthly-${uid}-${totalsResult.monthKey}`
    );
    push80IfCrossed(
      "quarterly",
      quarterlyLimit,
      totalsResult.quarterly_total,
      prevQuarterlyTotal,
      `80-quarterly-${uid}-${totalsResult.quarterKey}`
    );
    ...
    for (const w of periodWarnings80) {
      ...
      const warnPayload: Record<string, string> = {
